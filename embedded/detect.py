...
# GPIO 핀 설정
LED1_PIN = 17  # 병을 인식했을 때 켜질 LED 핀 번호
LED2_PIN = 27  # 캔을 인식했을 때 켜질 LED 핀 번호
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if cntb==2:
                print("YESBOTTLE")
                request_thread = threading.Thread(target=send_request2)
                request_thread.start()
            if s=='0: ':
                cntb=0
                cntc=0
